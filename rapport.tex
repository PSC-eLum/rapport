...
\begin{tabular}{l c r}
  Facteur & Notation & Correlation \\
  Measured Solar Irradiance & IrrPOA(W/m2) & 0.978788 \\
  Temperature dans l’air & T(C) & 0.394982 \\
  Wind Speed & WS(m/s) & 0.133656 \\
  Wind Direction & WD(deg) & -0.049585 \\
  Solar Zenith Angle & SZA(deg) & -0.667123 \\
  Solar Azimuth Angle & SAA(deg) & 0.040420 \\
\end{tabular}
...
\begin{figure}
\centering
\includegraphics[width=0.7\linewidth]{./categoriesgraph}
\caption{K-Means algorithme avec 3 clusters}
\label{fig:categoriesgraph}
\end{figure}
...
Deux algorithmes principales étaient utilisés ensuite pour apprendre à prédire les données : Support Vector Machine (SVM) et Random Forest. 

L’algorithme SVM était utilisé avec trois noyaux différents - Lineaire, RBF, et polynomiale. Les résultats sont comparables à ceux de Random Forest, mais le temps pour trainer l’algorithme est considérable. La décision était donc prise de continuer avec le Random Forest. 

\subsection{SVM}
Linear R$\exp{2} $score
0.958624795379
RBF R$\exp{2}$ score
0.96155168192
Polynomial R$\exp{2}$ score
0.553626719589
